CREATE PROCEDURE [dbo].[sps_Invoices_GetAllByClientId]
	@BusinessId int,
	@ClientId int,
	@Page int = 0,
	@Search NVARCHAR(50) = '',
...
BEGIN
	...
	BEGIN
		...
		FROM Invoices inv
		WHERE inv.[BusinessId] = @BusinessId AND inv.ClientId = @ClientId AND inv.[IsActive] = 1
	...
	BEGIN
		...
		SELECT inv.[Id],inv.[CreateBy],inv.[InvoiceNumber],inv.[Description],inv.[InvoiceDate],inv.[StatusId],inv.[ClientId],inv.[ClientEmail],inv.[DueDate],inv.[IsOverdue],inv.[Total]
		FROM Invoices inv
		WHERE inv.BusinessId = @BusinessId AND inv.ClientId = @ClientId AND inv.IsActive = 1 AND
